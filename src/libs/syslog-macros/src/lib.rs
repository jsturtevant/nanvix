...
const CHATTY_LIBCALL_PREFIXES: [&str; 2] = ["pthread_", "sem_"];
const CHATTY_LIBCALL_NAMES: [&str; 1] = ["__errno_location"];
...
fn should_skip_instrumentation(function: &ItemFn, kind: TraceKind) -> bool {
    if !matches!(kind, TraceKind::Libcall) {
        return false;
    }

    let ident_name: String = function.sig.ident.to_string();

    if CHATTY_LIBCALL_NAMES.iter().any(|name| ident_name == *name) {
        return true;
    }

    CHATTY_LIBCALL_PREFIXES
        .iter()
