    ...
    pub unsafe fn dispatch(
            ...
            .find(|(_, slot)| slot.is_free())
        {
            ...
            None => {
                ...
                // panicking, we fail gracefully. We restore the `available_slots` semaphore count
                // because it was decremented before we attempted to find a free slot, but none was
                // actually acquired.
                ...
                if let Err(rollback_error) = unsafe { scoreboard.available_slots.up() } {
                    let rollback_reason: &str =
                        "failed to restore available scoreboard slots after no free slot was found";
