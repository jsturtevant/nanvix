...
pub unsafe fn push(
    ...
    // TODO: implement push().
    // 1. Ensure destination process/thread are valid
    // 2. Check if destination is waiting to receive data from caller
    // 2.1. If it is, copy data from caller to destination buffer, wake up destination.
    // 2.2. If it is not, block caller until destination is ready to receive data.

    let reason: &str = "not implemented";
    warn!(
        "{reason} (caller_pid={caller_pid:?}, caller_tid={caller_tid:?}, \
         destination_pid={destination_pid:?}, destination_tid={destination_tid:?}, \
         buffer_ptr={buffer_ptr:?}, transfer_len={transfer_len})"
    );
    Err(SleepError::Generic(Error::new(ErrorCode::OperationNotSupported, reason)))
}
