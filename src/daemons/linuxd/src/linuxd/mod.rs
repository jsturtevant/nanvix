    ...
    fn trap_if_pending_snapshot(&self) -> Result<()> {
        if self.in_l2 {
            ...
            let mut trap_listener: SocketListener =
                Socket::bind(SocketType::Tcp, nanvixd::config::restore_gate_sockaddr_builder())?;
            ...
            'poll_loop: loop {
                ...
                // script.
                println!("Linuxd hit pre-snapshot trap");
                ...
                match event.token() {
                    ...
                    CONTROL_PLANE_TOKEN => {
                        let cmd: control_plane::Command =
                            match control_plane::try_read_command(&mut control_plane_stream) {
                                Ok(cmd) => cmd,
